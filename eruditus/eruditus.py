...
class CTFReactionHandler(commands.Cog):
    """CTF 참가 반응을 처리하는 Cog"""
    
    def __init__(self, bot, ctf_group):
        self.bot = bot  # 봇 인스턴스 저장
        self.ctf_group = ctf_group  # CTF 클래스의 인스턴스 저장
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        # 저장된 join 메시지인지 확인
        ctf_info = self.ctf_group.ctf_join_messages.get(payload.message_id)
        if not ctf_info:
            return
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        # 반응 제거 시 역할도 제거
        ctf_info = self.ctf_group.ctf_join_messages.get(payload.message_id)
        if not ctf_info or str(payload.emoji) != "⚔️":
            return
